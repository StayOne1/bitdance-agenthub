...
| **CustomAgentAdapter** | ✅ | OpenAI Chat Completions 兼容协议，接 DeepSeek / OpenAI / 火山方舟 |
...
支持第三方 API 网关（per-agent `apiBaseUrl` + token），但按 adapter 分协议：Claude Code 需要 Anthropic 兼容 endpoint，Custom 需要 OpenAI Chat Completions 兼容 endpoint，Codex 需要 Codex/Responses 兼容 endpoint。DeepSeek 没有 `/responses`，应走 CustomAgentAdapter。
