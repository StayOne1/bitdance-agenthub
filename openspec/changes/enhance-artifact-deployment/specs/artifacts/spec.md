...
Each ready local deployment of a `web_app` artifact MUST write a static package under AgentHub-managed data storage and serve it from a stable deployment URL.

#### Scenario: User opens a local deployment URL
- **WHEN** the user opens `/deployments/{deploymentId}`
- **THEN** AgentHub serves the materialized web app entry document
...
- **THEN** AgentHub returns a ZIP file for the materialized deployment.
