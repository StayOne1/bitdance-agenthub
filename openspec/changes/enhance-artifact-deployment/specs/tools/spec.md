...
AgentHub MUST provide a `deploy_artifact` tool that accepts a web app artifact id and returns a deployment status record with a stable preview path. Successful local deployments MUST materialize static files under AgentHub-managed data storage and expose source/container package download paths.
...
- **AND** no files are written outside AgentHub deployment storage.
